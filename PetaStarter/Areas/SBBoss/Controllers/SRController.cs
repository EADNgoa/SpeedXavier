...
namespace Speedbird.Areas.SBBoss.Controllers
{
    public class SRController : EAController
    {
        ...
        private string[] SRDetColumns => new string[]
       {
        ...
        public ActionResult Manage([Bind(Include = "SRID,BookingNo,CustID,SRStatusID,EmpID,BookingTypeID,EnquirySource,AgentID,TDate,PayStatusID,ServiceTypeID,AgentBooker")] ServiceRequest item, string Event, int? CID, string FName, string SName, string Email, string Phone, string routeto,bool isLead=false)
        {
            using (var transaction = db.GetTransaction())
            {
                ...
                {                    
                    ...
                    if (routeto == "BFSave" && agentId != null)
                    {
                        if (item.AgentID?.Length > 0 && TotalSell <= creditAmt)
                        {
            ...
            switch (sType)
            {
                case ServiceTypeEnum.Accomodation:
                    return PartialView($"ReadPVs/_{(sType).ToString()}", db.SingleOrDefault<AccomodationServiceView>($"select (select top 1 CONCAT(c.fName, ' ',c.sName) from Customer c, " +
                        ...
                        $"Model as AccomName, sd.IsCancelled, Fdate as checkin, Tdate as checkout, sd.AdultNo, ChildNo, InfantNo," +
                ...
                case ServiceTypeEnum.Packages:
                    return PartialView($"ReadPVs/_{(sType).ToString()}", db.SingleOrDefault<Packagevw>($"SELECT (select top 1 CONCAT(c.fName, ' ',c.sName) from Customer c, SRD_Cust sc " +
                        $"where c.CustomerID = sc.CustomerID and sc.SRDID = { srdid}) as PaxName, sd.ChildNo as NOOfPax,sd.IsCancelled as Cancelled, " +
                ...
                case ServiceTypeEnum.SightSeeing:
                    ...
                    var qry = $"select (select top 1 CONCAT(c.fName, ' ',c.sName) from Customer c, SRD_Cust sc " +
                        ...
                        $"LEFT JOIN DriversCars dc ON dc.CarId = NoExtraBeds " +
                         $"left join Refunds rf on rf.SRDID = sd.SRDID " +
                        $"WHERE sd.SRDID = { srdid } ";
                ...
                case ServiceTypeEnum.CarBike:
                    return PartialView($"ReadPVs/_{(sType).ToString()}", db.SingleOrDefault<CarBikevw>($"select (select top 1 CONCAT(c.fName, ' ',c.sName) from Customer c, " +
                        ...
                        $"left join OptionType ot on ot.OptionTypeId = sd.OptionTypeId " +
                         $"left join Refunds rf on rf.SRDID = sd.SRDID " +
                        $"LEFT JOIN DriversCars dc ON dc.CarId = NoExtraBeds WHERE sd.SRDID = {srdid}"));
                    break;
                ...
                case ServiceTypeEnum.Rail:
                    return PartialView($"ReadPVs/_{(sType).ToString()}", db.SingleOrDefault<Railvw>($"SELECT (select top 1 CONCAT(c.fName, ' ',c.sName) from Customer c, SRD_Cust sc " +
                        $"where c.CustomerID = sc.CustomerID and sc.SRDID = { srdid}) as PaxName, sd.ChildNo as Age,sd.IsCancelled as Cancelled, " +
                        $"sd.DateOfIssue as DOT, sd.FromLoc,sd.IsCancelled, sd.ToLoc,sd.AdditionalDetails as AddDetl,ot.OptionTypeID,ot.OptionTypeName," +
        ...
        public PartialViewResult FetchDailyRecords(ServiceTypeEnum sType, int srdid)
        {
            //First Fetch the uploads
            ViewBag.ufs = db.Query<SRUpload>("Where srdid=@0", srdid);
            ...
            switch (sType)
            {
                case ServiceTypeEnum.Accomodation:
                    ...
                    return PartialView($"ReadDailyPVs/_{(sType).ToString()}", db.SingleOrDefault<AccomodationServiceDaily>($"select (select top  1 CONCAT(c.fName, ' ',c.sName, ' ',c.Phone) from Customer c, SRD_Cust sc where c.CustomerID = sc.CustomerID and sc.SRDID = {srdid}) as PaxName," +
                        $"ag.AgentID,ag.ContactName,ag.PhoneNo,aus.RealName as Agency,aus.Id,srq.BookingNo,srq.TDate,sd.AdultNo, CarType as ExtraBedCost, ChildNo, ContractNo, cost, sd.CouponCode, ECommision, sd.Fdate as checkin, sd.Tdate as checkout, FromLoc, " +
                        $"HasAc, Heritage as ExtraService, InfantNo, Model as AccomName, ot.OptionTypeName as RoomCategory, " +
                ...
                case ServiceTypeEnum.Packages:
                    ...
                    return PartialView($"ReadDailyPVs/_{(sType).ToString()}", db.SingleOrDefault<PackageDaily>($"select (select top 1 CONCAT(c.fName, ' ',c.sName,' ',c.Phone) from Customer c, " +
                        ...
                        $"ag.AgentID,ag.ContactName,aus.RealName as Agency,aus.Id,ag.PhoneNo,srq.BookingNo,srq.TDate as TransDate,sd.ChildNo as NOOfPax, sd.Model as PackageType, sd.Fdate, sd.Tdate," +
                ...
                case ServiceTypeEnum.Cruise:
                    ...
                    return PartialView($"ReadDailyPVs/_{(sType).ToString()}", db.SingleOrDefault<CruiseDaily>($"select (select top 1 CONCAT(c.fName, ' ',c.sName, ' ',c.Phone) from Customer c, " +
                        ...
                        $"(select COUNT(c.CustomerID) from Customer c, SRD_Cust sc " +
                        $"where c.CustomerID = sc.CustomerID and sc.SRDID = { srdid}) as PaxNo, " +
                        $"ag.AgentID,ag.ContactName,aus.RealName as Agency,aus.Id,ag.PhoneNo,srq.BookingNo,srq.TDate,sd.Qty as Passengers, " +
                ...
                case ServiceTypeEnum.SightSeeing:
                    ...
                    var qry = $"select (select top 1 CONCAT(c.fName, ' ',c.sName, ' ',c.Phone) from Customer c, SRD_Cust sc where c.CustomerID=sc.CustomerID and sc.SRDID={srdid}) as PaxName, " +
                        $"ag.AgentID,ag.ContactName,ag.PhoneNo,aus.RealName as Agency,aus.Id,srq.BookingNo,srq.TDate,AdultNo,ChildNo,sd.Model as SightseeingName, sd.OptionTypeID, ot.OptionTypeName,PickUpPoint,FromLoc as PickupLocation, Heritage as Private_Sic, DinnerCost as CostPerCar,Cost, " +
            ...
            "SuppInvAmt,GDSConfNo,ExpiryDate,EBCostPNight,Name,AdditionalDetails")] SRdetail item, string Event, int CustomerId)
        {
            using (var transaction = db.GetTransaction())
            {
                ...
                try
                {
                   ...
                   if(agentId != null)
                    ...
                    if (agentId != null)
                    {
                        if (TotalSell <= creditAmt)
                        {
                            db.Execute($"Update ServiceRequest set SRStatusID = {(int)SRStatusEnum.Confirmed}");
                        }
                        else
                        {
                            db.Execute($"Update ServiceRequest set SRStatusID = {(int)SRStatusEnum.Unconfirmed}");
                        }
                    }
            ...
            return RedirectToAction("Manage", new { id = item.SRID, mode = 3 , ViewBag.NCredit });
        }
        ...
        [ValidateAntiForgeryToken]
        [EAAuthorize(FunctionName = "Service Requests", Writable = true)]
        public async System.Threading.Tasks.Task<ActionResult> SRCanxServiceAsync([Bind(Include = "RefundId,SRDID,ProdCanxCost,SBCanxCost,Note")] Refund item)
        {
            using (var transaction = db.GetTransaction())
            {
                ...
                var srid = db.ExecuteScalar<int>("Select SRID from SRdetails where SRDID = @0", item.SRDID);

                //If the supplier has been paid then adjust his payment amount too.
                var supPayRecs = db.Query<DRP_SR>("where SRID=@0", srid);
                ...
                var atompaymentlog = db.SingleOrDefault<int>("Select SRDID from AtomPaymentLogs where SRDID = @0", item.SRDID);

                if(atompaymentlog == item.SRDID)
                {
                ...
                var customer = db.SingleOrDefault<ServiceCustomervw>("select CONCAT(cu.FName, ' ', cu.SName) as FullName, cu.Email, cu.Phone, src.IsLead from SR_Cust src " +
                        ...
                        "where src.ServiceRequestID = @0 and IsLead = 1", srid);

                var config = db.FirstOrDefault<Config>("select merchantid,pwd from config");
                var atomtxn = db.SingleOrDefault<AtomPaymentLog>("select rmmp_txn,TDate from AtomPaymentLogs where srid = @0", srid);
                ...
                db.Insert(item);
                transaction.Complete();
                return RedirectToAction("Manage", new { id = srid, mode = 5, ViewBag.Response, ViewBag.SResponse });
            }
